...
function initializeMap() {
  ...
  const crs = L.TileLayer.Swiss.EPSG_2056;
  ...
  const { bounds } = crs.options;
  const padding = -50000;
  map.fitBounds([
    crs.projection.unproject(bounds.min.subtract([padding, padding])),
    crs.projection.unproject(bounds.max.add([padding, padding])),
  ]);
